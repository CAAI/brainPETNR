...
from utils import _MidPointNorm, lr_model
from mpl_toolkits.axes_grid1.inset_locator import inset_axes
from mpl_toolkits.axes_grid1 import ImageGrid
from matplotlib.gridspec import GridSpec

curr_folder = pathlib.Path(__file__).parent.resolve()
plt.style.use(curr_folder.joinpath('plots.mplstyle'))
...
def brain_slices_grid_pib(data,
    ...
    # Lasses way of normalizing in the PiB reports
    # normalization
    ...
    titles = ['low activity', 'standard activity', 'de-noised']
    ...
    for idx, slice_i in enumerate(slices):
        ...
        for idy in range(num_cols):
            ...
            im = ax[idx, idy].imshow(data2d,
                                     cmap=col,
                                     aspect='equal',
                                     vmin=v_scale[0],
                                     vmax=v_scale[1])  #, norm=normalization)
            ...
            if segment_data.any():
                for j in range(0, segment_data.shape[0]):
                    ...
                    if mask2d.any():
                        ...
                        colors = 'yellow' if j == segment_data.shape[
                            0] - 1 else 'white'
                        styles = {
                            'colors': colors,
                            'linewidths': 0.8
            ...
            if idx == num_rows - 1:
                ax[idx, idy].text(0.03 * plot_extent,
                                  0.96 * plot_extent,
                                  titles[idy],
                                  color='white',
                                  fontsize=14)
    ...
    if metrics:
        ax[0, 0].text(
            0.05 * plot_extent,
            0.96 * plot_extent,
            f"Cortical uptake ratio (SUVr) = {np.round(metrics['lowdose'], 2)}",
            color='white',
            fontsize=14)
        ax[0, 1].text(0.75 * plot_extent,
                      0.96 * plot_extent,
                      np.round(metrics['true'], 2),
                      color='white',
                      fontsize=14)
        ax[0, 2].text(0.75 * plot_extent,
                      0.96 * plot_extent,
                      np.round(metrics['infer'], 2),
                      color='white',
                      fontsize=14)
...
def brain_slices_grid_pe2i(data,
    ...
    # Lasses way of normalizing in the PE2I reports
    normalization = _MidPointNorm(vmin=-1,
                                  vmax=0.9 * data[1].max(),
                                  midpoints=[0],
                                  defmids=[0.07])
    ...
    for idx, slice_i in enumerate(slices):
        ...
        for idy in range(num_cols):
            ...
            im = ax[idx, idy].imshow(
                data2d, cmap=col, aspect='equal',
                norm=normalization)  #vmin=v_scale[0], vmax=v_scale[1], norm=)
    ...
    ax[0, 0].text(0.03 * plot_extent,
                  ...
                  fontsize=14)
    # CP ratio
    ax[0, 0].text(0.6 * plot_extent,
                  0.96 * plot_extent,
                  f"r_CP = {np.round(metrics['lowdose'], 2)}",
                  color='white',
                  fontsize=14)
    ax[0, 1].text(0.75 * plot_extent,
                  0.96 * plot_extent,
...
def pixelvalue_jointplot_log(
        ...
        fname='',
        nbins=200,
        crop_th=0.5):  #fname, ylabel='Pixel value', color='tab:blue'):
...
def box_roi_percent_diff_images(x_ld, x_true, x_infer, fname, extent=0):
    ...
    ax11.imshow(x_true[:, soi, :],
                cmap=_PETRainbowCMAP,
                aspect='equal',
                vmin=0,
                vmax=x_true[:, soi, :].max() * 0.99)
    ...
    ax21.imshow(percent_diff_lowdose_true_full[:, soi, :],
                cmap='hot',
                aspect='equal',
                vmin=0,
                vmax=100)
    ...
    im = ax31.imshow(percent_diff_infer_true_full[:, soi, :],
                     cmap='hot',
                     aspect='equal',
                     vmin=0,
                     vmax=100)
...
def bland_altman_plot(df, dirname, fname='', xlim=None, ylim=None):
    ...
    ax = sns.scatterplot(x=data['mean_true_ld'],
                         y=data['diff_true_ld'],
                         label='lowdose - true',
                         color=colors[0])
    sns.scatterplot(x=data['mean_true_infer'],
                    y=data['diff_true_infer'],
                    label='AI - true',
                    color=colors[1])
...
def boxplot_from_dataframe(df, dirname, fname, orient='h'):
    ...
    Args:
        ...
        fname ([type]): [description]
        orient (str, optional): [description]. Defaults to 'h'.
    """
...
def ssmi_psnr_nrmse_plot(df, dirname, fname):
    ...
    for j, kw in enumerate(['ssmi', 'nrmse', 'psnr']):
        sns.boxplot(ax=ax[0, j],
                    data=df[df['metric'] == kw],
                    x='metric',
                    y='value',
                    hue='phase',
                    palette=palette)
        ...
        if j == 1:
            ax[0, j].legend(handles=handles[0:],
                            labels=labels[0:],
                            frameon=False,
                            fontsize=14)
        else:
...
def lmplot_compare(df, dirname, fname, thresh=None, xlim=None, ylim=None):
    ...
    sns.regplot(data=df,
                ...
                y='lowdose',
                color=colors[0],
                label=f'low-dose: {lr_ld[0]} +/- {lr_ld[1]}')
    ax = sns.regplot(data=df,
                     x="true",
                     y='infer',
                     color=colors[1],
                     label=f'AI: {lr_ai[0]} +/- {lr_ai[1]}')
    if thresh:
