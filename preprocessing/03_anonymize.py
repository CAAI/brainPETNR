...
def anonymize(origin_folder, dest_folder, an_id):
    ...
    cmd_list = [r'C:\JSRecon12\anonymize.exe', '-i', str(origin_folder), '-o', str(dest_folder), '-n', an_id, '-p', '-u']
    run(cmd_list)
        
    step_failed = len(os.listdir(dest_folder)) != len(os.listdir(origin_folder))
    time.sleep(1)
    return step_failed
...
json_file = raw_data_dir.joinpath(f"{project_id}_patient_to_anonym_id.json")
...
for patient_folder, anonym_id in tqdm(patient_to_anonym_id.items()):
    ...
    if anonymization_failed:
        print(f"Error anonymizing {patient_folder} -> {anonym_id}. Retrying in the end")
...
for patient_folder, anonym_id in failed_anonymization.items():
    ...
    if anonymization_failed:
        ...
        print(f"Error anonymizing for the 2nd time: {patient_folder} -> {anonym_id}. Storing info")
        with open(f'{project_id}_anonymization_log.txt', 'a') as lf:
            lf.write(f"""Error anonymizing some files from {patient_folder} -> {anonym_id}
...
if failed_anonymization_final:
    json_file = anonym_data_dir.joinpath(f'{project_id}_failed_anonymizations.json')
