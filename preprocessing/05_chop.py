...
def main():
    ...
    parser.add_argument(
        ...
        "--chop-percent",
        help=
        "Percent for lowdose chopping. Use space to separate multiple values e.g. 10 20",
        nargs='*',
        type=int,
        default=[])
    parser.add_argument(
    ...
    if fixed_lowdose_value:
        df[f'LD{fixed_lowdose_value}MBq'] = df['tracer dose (MBq)'].apply(
            lambda x: [round(fixed_lowdose_value / x * 100, 2)])
    elif percent:
        for pc in percent:
            df[f'LD{pc}pct'] = pc
    else:
        raise ValueError(
            'Must define --chop-percent or --fixed-lowdose. Use -h for help.')
    ...
    for anonym_folder in tqdm(anonym_folders):
        ...
        lowdose_anonym_folder = data_dir.joinpath(anonym_id).joinpath(
            f"PET_LD{percent[-1]}pct")

        # check if data already been chopped
        if lowdose_anonym_folder.exists():
            if len(os.listdir(lowdose_anonym_folder)) == len(
                    os.listdir(pet_folder)):
        ...
        if fixed_lowdose_value:
            ...
            print(f"Chopping {anonym_id} with dose {full_dose_value} MBq by",
                  *percent, "%")

        chop_cmd_list = [
            "cscript", "C:\JSRecon12\LMChopper64\LMChopper64.js",
            str(lm_file)
        ]
        run(chop_cmd_list)
        ...
        if len(percent) > 1:
            ...
            for counter, pc in enumerate(percent):
                folder_pc = data_dir.joinpath(anonym_id).joinpath(
                    f"PET_LD{pc}pct")
                ignore_files = [
                    f.name for f in lowdose_anonym_folder.iterdir()
                    if lm_file.stem in f.name and f"-{pc:03}.000" not in f.name
                ]
                if counter != len(percent) - 1:
                    print(
                        f"Copying {lowdose_anonym_folder.name} -> {folder_pc.name}"
                    )
                    copytree(lowdose_anonym_folder,
                             folder_pc,
