...
def time_delay(col):
    return pd.to_timedelta(col['study time'] - col['tracer injection time']).seconds

def delay_for_recon(col, base_duration):
    if col['image duration (sec)'] <= base_duration:
        return 'all'
    else:
        # if delay for patient above mean then 0
        recon_delay =  np.maximum(0, avg_delay_at_mode - col['time delay (sec)'])
        # if delay for recon is too long
        recon_delay = np.minimum(recon_delay, col['image duration (sec)'] - base_duration)
        return f'{int(recon_delay)}:{int(base_duration)}'
        
# maps 
scanner_type = {'RHPET6NAV': 'TRUEPOINT', 
                'RHKFASIEPTR5': 'VISION',
                'RHPET5NAV': 'TRUEPOINT',
                'RHKFASIEPTR3': 'VISION',
                'RHPET4NAV': 'MCT',
...
parser.add_argument("-p", "--project-id", 
                    help="Project keyword or ID to prefix the patient database. Default is parent folder name.", 
                    type=str, default='')
...
folders = [f for f in anonym_data_dir.iterdir() if f.is_dir() and 'lowdose' not in f.name]
...
for col_name in ['study', 'tracer injection']:
    df[f'{col_name} time'] = df.apply(lambda col: format_date(col[f'{col_name} date (yyyy:mm:dd)'], 
                                                              col[f'{col_name} time (hh:mm:ss GMT+00:00)']), axis=1)
...
df['tracer dose (MBq)'] = df['tracer activity at time of injection (Bq)'].apply(lambda x: float(x)/1e6)
...
avg_delay_at_mode = df[df['image duration (sec)'] == duration_mode]['time delay (sec)'].mean()
# find the starting value for recon
df['delay for recon'] = df.apply(delay_for_recon, args=(duration_mode,), axis=1)

# map the StationName onto ScannerName
df['ScannerName'] = df['StationName'].map(scanner_type)

# export dataframe as csv
df.to_csv(anonym_data_dir.joinpath(f'{project_id}_anonymized_patient_info.csv'))