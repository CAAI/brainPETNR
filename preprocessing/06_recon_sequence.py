...
def e7_sequence(input_dir, output_dir, param_file, recon_type, is_test):
    ...
    data_dir_converted = input_dir.parent.joinpath(f"{input_dir.name}-Converted")
    ...
    subprocess.run(["cscript", "C:\\JSRecon12\\JSRecon12.js", str(input_dir), str(param_file)])
    ...
    if is_test:
        ...
        hf_content = [l for l in hf_content if 'scale factor (mm/pixel) [1]' in l]
        print(*hf_content)
        print('Remember to check zoom value in param file to calculate accordingly!')
        sys.exit(-1)
    
    # running IF2Dicom.js
    subprocess.run(["cscript", "C:\\JSRecon12\\IF2Dicom.js", hdr_file, f"Run-05-{input_dir.name}-LM-00-IF2Dicom.txt"])
    
    # Copying the DICOM folder over to base directory
    dicom_folder = LM00_folder.joinpath(f"{input_dir.name}{hdr_file_naming}.v-DICOM")
...
def main():
    parser = argparse.ArgumentParser(description='Runs the JSRecon12 reconstruction sequence on all folders in a given data directory.')
    parser.add_argument("-i", "--input", 
                        help="Directory containing patients data. Will use current working directory if nothing passed", 
                        type=str, default=os.getcwd())
    parser.add_argument("-o", "--output", 
                        help="Output directory for recon data. Default is 'data_recon' if nothing passed.", 
                        type=str, default='data_recon')
    parser.add_argument("-c", "--config",
                        help="Config file for recon",
                        type=str, default=Path(os.getcwd()).parent.joinpath('recon_config.yaml'))
    parser.add_argument("-p", "--params", 
                        help="Path to params file. Detects params file containing project name if none given", 
                        type=str, default='')
    ...
    if args.params:
        e7_params = Path(args.params)
    else:
        raise FileNotFoundError(""" Could not find params file for JSRecon to run. Need to pass as argument.""")
    ...
    df_filter = df_filter[df_filter['image duration (sec)'] > int(0.9 * duration_mode)]

    good_patients = df_filter.index
    patient_folders = [d for d in data_dir.iterdir() if d.is_dir() if d.name in good_patients]
    if is_test:
    ...
    for pf in tqdm(patient_folders):
        ...
        # pe2i 270:60 (4min30sec:1min)     285:30 (0.5 min)
            ...
            output_folder = data_folder.name + f'_LD{acq_time}sec{delay_time}delay'
        ########################################### 
        output_folder = recon_data_dir.joinpath(patient_id).joinpath(output_folder)
        # create temporary params file
        temp_params_file = temp_folder.joinpath(f"{patient_id}_{output_folder.name}_{e7_params.stem}")
        edit_params_file(e7_params, temp_params_file, delay_for_recon)
                
        try:
            e7_sequence(data_folder, output_folder, temp_params_file, recon_type, is_test)
    ...
    if fails:
        ...
        json_file = data_dir.joinpath(f'{project_id}_failed_reconstructions_with_code.json')
