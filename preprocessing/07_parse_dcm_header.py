...
def main():
    parser = argparse.ArgumentParser(description=""" Parse all info contained in PET and CT dicom files (after reconstruction).
                                                 Exports info for all patients to one json file.""")
    parser.add_argument("-i", "--input", 
                        help="Directory containing reconstructed patients data. Will use current working directory if nothing passed", 
    ...
    for patient_folder in tqdm(patient_folders):
        ...
        pet_folder = patient_folder.joinpath('PET')
        dcm_files = [f for f in pet_folder.iterdir() if '.dcm' in f.name or '.ima' in f.name]
    ...
    df['Activity (MBq)'] = df['RadionuclideTotalDose'].apply(lambda x: int(x) / 1e6)
    ...
    scaling = {'PET': 1.0, 'CT': 1.0, 'MR': 1.0}
    activity = {'PET': 1.0, 'CT': 1.0, 'MR': 1.0}
    for name in pet_files:
        if name in scaling:
            continue
        if 'sec' in name:
            delay = int(name.split('sec')[-1].replace('delay', ''))
            frame_duration = int(name.split('sec')[0].replace('PET_LD', ''))
            scaling[name] = 1.0 / (np.exp(-delay*np.log(2)/radionuclide_halflife))
            activity[name] = frame_duration / base_frame_duration
        elif 'pct' in name:
            pct = int(name.replace('pct', '').replace('PET_LD', ''))
