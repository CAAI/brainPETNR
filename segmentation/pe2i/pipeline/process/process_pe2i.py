...
from nipype.interfaces.fsl import (FLIRT, Threshold, BET, IsotropicSmooth,
                                   ConvertXFM)
...
class JobProcess(object):
    ...
    def __init__(self, job):
        self._job = job
        self.wf = Workflow(name="PETCTPE2I_segmentation",
    ...
    def _setup_posterior_left_right_putamen(self):
        ...
        self.extract_putamen_posterior_left = Node(
            interface=Function(input_names=["putamen_filename"],
                               output_names=["out"],
                               function=extract_putamen_posterior.extract),
            name="extract_putamen_posterior_left")

        self.wf.connect(self.left_seg, "out_file",
                        self.extract_putamen_posterior_left,
                        "putamen_filename")

        self.extract_putamen_posterior_right = Node(
            interface=Function(input_names=["putamen_filename"],
                               output_names=["out"],
                               function=extract_putamen_posterior.extract),
        ...
        self.wf.connect(self.right_seg, "out_file",
                        self.extract_putamen_posterior_right,
                        "putamen_filename")
    ...
    def _setup_putamen_caudate(self):
        ...
        *both* hemisphere collectively.
        ...
        nn_webservice = Node(interface=Function(
            function=nn_predict,
            input_names=["ct_file", "pet_file"],
            output_names=["out_file"]),
                             name="nn_webservice")

        self.wf.connect(self.pet_to_avg, "out_file", nn_webservice, "pet_file")

        self.wf.connect(self.ct_to_avg, "out_file", nn_webservice, "ct_file")
        ...
        self.wf.connect([(nn_webservice, self.nn_segmentation, [("out_file",
                                                                 "in_file")]),
    ...
    def _preliminary_setup(self):
        ...
        self.convert_func = Node(interface=Dcm2niix(), name='dcm_conv_func')
        ...
        self.pet_to_ct = Node(interface=FLIRT(), name='pet_to_ct')
        ...
        self.wf.connect([(self.convert_struct, self.pet_to_ct,
                         ...
                         (self.convert_func, self.pet_to_ct,
        ...
        self.ct_to_avg.inputs.reference = os.path.join(settings.ETC_DIR,
                                                       settings.TEMPLATE)
        ...
        self.wf.connect([(self.ct_to_avg, self.ct_to_avg_nonbet,
                          [("out_matrix_file", "in_matrix_file")]),
                         (self.convert_struct, self.ct_to_avg_nonbet,
                          [("converted_files", "in_file")])])

        seg_to_pet_xfm = Node(interface=ConvertXFM(), name="seg_to_pet_xfm")
        seg_to_pet_xfm.inputs.concat_xfm = True
        self.wf.connect([(self.ct_to_avg, seg_to_pet_xfm, [("out_matrix_file",
                                                            "in_file2")]),
                         (self.pet_to_ct, seg_to_pet_xfm, [("out_matrix_file",
                                                            "in_file")])])

        self.pet_to_avg = Node(interface=FLIRT(), name="pet_to_avg")
        self.pet_to_avg.inputs.apply_xfm = True
        self.wf.connect([(seg_to_pet_xfm, self.pet_to_avg, [
            ("out_file", "in_matrix_file")
        ]), (self.ct_to_avg, self.pet_to_avg, [("out_file", "reference")]),
                         (self.convert_func, self.pet_to_avg,
                          [("converted_files", "in_file")])])

        self.segment_lab4 = Node(interface=LabelFusion(), name="label4")
        ...
        self.wf.connect([(self.ct_to_avg, self.segment_lab4,
...
if __name__ == "__main__":
    ...
    patients = [
        d for d in os.listdir(settings.DATA_DIR)
        if os.path.isdir(os.path.join(settings.DATA_DIR, d))
    ]

    for c, data_id in enumerate(patients):
        print "processing patient {}/{} with id {}".format(
            c + 1, len(patients), data_id)
        job = Job(data_id)
        p = JobProcess(job)
        p.start()
