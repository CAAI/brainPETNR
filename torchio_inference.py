...
def main():
    ...
    test_subjects = data_module.prepare_patient_data('test')
    ...
    if 'cropping' in configs:
        x_min, x_max = configs['cropping']['x_lim']
        y_min, y_max = configs['cropping']['y_lim']
        z_min, z_max = configs['cropping']['z_lim']
    ...
    for patient in tqdm(test_set):
        ...
        if not output_file.exists():
            ...
            full_volume = infer_data_from_model(
                model, patient, patch_size, patch_overlap, gpu= args.gpu)
            ...
            final_image = tio.ScalarImage(tensor=full_volume, affine=patient.input0.affine)
            
            # pad if there was cropping
            if 'cropping' in configs:
                crop_config = configs['cropping']
                pad = tio.Pad((*crop_config['x_lim'], *crop_config['y_lim'], *crop_config['z_lim']))
            ...
            print(f'Data for {patient.id} already reconstructed with model {model_name}')
